...
# --- 全局配置 ---
# 注意：在大数据量下，不要使用全局 DataFrame 存储所有数据，也不要使用 dcc.Store 存储百万级行数据。
# 必须使用 "按需查询 (Query on Demand)" 模式。
...
app.title = "ABM Analysis (Big Data Mode)"
...
app.layout = dbc.Container(fluid=True, className="p-4 bg-light", children=[
    ...
    html.H1("ABM Simulation Analysis (Direct SQL Mode)", className="mb-4"),
    ...
    dbc.Tabs(id="main-tabs", active_tab="tab-market", children=[
        ...
        dbc.Tab(label='4. Agent Stats', tab_id='tab-agent-type', children=[
            dbc.Card(dbc.CardBody([
                html.H3("Agent Type Analysis"),
                dcc.Loading(dcc.Graph(id='agent-type-asset-chart')),
                html.Hr(),
                html.H4("Daily Statistics"),
...
def connect_database(n_clicks, db_path):
    ...
    if not conn:
        return None, dbc.Badge("Failed", color="danger"), 1, {}, 1, {}, 1, {}, 1, {}, [], []
    ...
    try:
        ...
        # 获取 Trader 列表 (限制数量，防止下拉框爆炸，或者只加载 ID 和 Type)
        # 这里为了性能，只加载 ID，Label 可以在前端显示
        ...
        return (db_path, dbc.Badge("Connected", color="success"),
                max_day, marks, max_day, marks, max_day, marks, max_day, marks,
                stock_opts, trader_opts)
    except Exception as e:
        print(f"Init Error: {e}")
        return None, dbc.Badge("Error", color="danger"), 1, {}, 1, {}, 1, {}, 1, {}, [], []
...
def update_market_tab(day, db_path):
    ...
    # 图表：读取所有市场数据 (Market Log 通常较小，1000行可以一次读取)
...
def update_stock_tab(stock_id, day, db_path):
    ...
    if row.empty:
        ...
        s = row.iloc[0]
        data = [
            ("Close", format_num(s['close'])), ("PE (TTM)", format_num(s['pe_ttm'])),
            ("PB", format_num(s['pb_ratio'])), ("Volume", format_large_number(s['volume'])),
            ("Market Cap", format_large_number(s['total_market_cap'])), ("Turnover Rate", format_percent(s['turnover_rate']))
        ]
        details = create_details_table(data)
...
def update_trader_tab(trader_id, day, db_path):
    ...
    fig.add_trace(go.Scatter(x=df_assets['day'], y=df_assets['cash'], name='Cash', visible='legendonly'))
    fig.add_trace(go.Scatter(x=df_assets['day'], y=df_assets['stock_value'], name='Stocks', visible='legendonly'))
...
def update_agent_type_tab(day, db_path):
    ...
    fig = px.line(df_chart, x='day', y='total_assets', color='trader_type', title="Total Assets by Agent Type")
...
if __name__ == '__main__':
    # 【修复】使用 app.run() 替代废弃的 app.run_server()
    app.run(debug=True)